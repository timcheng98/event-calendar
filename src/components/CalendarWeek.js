...
import {
  View, Modal, TouchableOpacity, FlatList, Text, ScrollView
} from 'react-native';
...
export default class CalendarWeek extends Component {
  ...
  renderWeekItem(date, day, item, bgColor = '#FFFFFF') {
    return (
      <ScrollView contentContainerStyle={{flexGrow: 2, backgroundColor: bgColor, borderBottomWidth: 1 / 3, borderColor: '#cccccc', justifyContent: 'center', alignItems: 'center', flexDirection: 'row'}}
      key={uuidV4()}
      >
        ...
        {item.length > 0 ? (
          ...
          <TouchableOpacity
          style={{flex: 0.8, alignItems: 'center', justifyContent: 'center', paddingVertical: '5%', width: '100%' }}
          >
  ...
  render() {
    ...
    return (
      <View>
        ...
        key={uuidV4()}
        >
          {this.renderWeekItem(this.getDayFromWeek(moment().week(), moment().year(), 0), 'Sun', this.state.sun, '#FDFDFD')}
          {this.renderWeekItem(this.getDayFromWeek(moment().week(), moment().year(), 1), 'Mon', this.state.mon)}
          {this.renderWeekItem(this.getDayFromWeek(moment().week(), moment().year(), 2), 'Tue', this.state.tue, '#FDFDFD')}
          {this.renderWeekItem(this.getDayFromWeek(moment().week(), moment().year(), 3), 'Wed', this.state.wed)}
          {this.renderWeekItem(this.getDayFromWeek(moment().week(), moment().year(), 4), 'Thu', this.state.thu, '#FDFDFD')}
          {this.renderWeekItem(this.getDayFromWeek(moment().week(), moment().year(), 5), 'Fri', this.state.fri)}
          {this.renderWeekItem(this.getDayFromWeek(moment().week(), moment().year(), 6), 'Sat', this.state.sat, '#FDFDFD')}
        </ScrollView>
        <Modal
          animationType="slide"
          transparent
